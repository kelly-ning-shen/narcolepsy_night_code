...
from network import SquareSmall0_5min_S
...
MODE = f'squaresmall_{DURATION_MINUTES}min_zscore_shuffle_ROC_ss'
...
def LeaveOneSubjectOut(base):
    ...
    for subject in subjects_data:
        ...
        model = SquareSmall0_5min_S(n_channels=3,nepoch=nepoch)
    ...
    def __getitem__(self, index: int):
        ...
        if is_per_epoch: # only for 15min (already saved as 300*300)
            ...
            if DURATION_MINUTES == 0.5:
                signal_pic = signal_pic.reshape((3,50,60))
            elif DURATION_MINUTES == 1:
                signal_pic = signal_pic.reshape((3,60,100))
